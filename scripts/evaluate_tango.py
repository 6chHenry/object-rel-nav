...
def run(args):
        ...
        try:
            if not (path_meta.stem.split("__")[0] + "_") in bad_episodes:
                with open(str(path_meta), "r") as f:
                    ...
                    distance_end[i] = (
                        float(distance_end_str) if distance_end_str != "nan" else np.nan
                    )
                    stat = meta[-4].strip("\n").split("=")[1]
                    stat_strs.append(stat)
                    status[i] = np.equal(np.repeat(stat, 2), status_codes)
                    episodes.append(path_meta.stem)
                records = np.genfromtxt(str(path_csv), delimiter=",")[
                    1:, :
                ]  # remove first row
    ...
    result_string = (
        ...
        f"\ncollisions\n"
        f"collided_percent <= 5 = {(((collided_percent <= 0.05).sum() / num_good_samples) * 100):.2f}\n"
        f"collided_percent <= 10 = {(((collided_percent <= 0.1).sum() / num_good_samples) * 100):.2f}\n"
        f"collided_percent <= 20 = {(((collided_percent <= 0.2).sum() / num_good_samples) * 100):.2f}\n"
        f"collided_percent <= 30 = {(((collided_percent <= 0.3).sum() / num_good_samples) * 100):.2f}\n"
        f"collided_percent <= 40 = {(((collided_percent <= 0.4).sum() / num_good_samples) * 100):.2f}\n"
        f"collided_percent <= 50 = {(((collided_percent <= 0.5).sum() / num_good_samples) * 100):.2f}\n"
        f"collided_percent <= 60 = {(((collided_percent <= 0.6).sum() / num_good_samples) * 100):.2f}\n"
        f"collided_percent <= 70 = {(((collided_percent <= 0.7).sum() / num_good_samples) * 100):.2f}\n"
        f"collided_percent <= 80 = {(((collided_percent <= 0.8).sum() / num_good_samples) * 100):.2f}\n"
        f"collided_percent <= 90 = {(((collided_percent <= 0.9).sum() / num_good_samples) * 100):.2f}\n"
        f"collided_percent <= 100 = {(((collided_percent <= 1.0).sum() / num_good_samples) * 100):.2f}\n"
    )
    print(result_string)
...
def parse_args():
    ...
    parser.add_argument(
        "--path_results", type=str, help="Path to the folder for recording results"
    )
    parser.add_argument(
        "--plot_show", help="Show distance histogram", action="store_true"
    )
    parser.add_argument(
        "--plot_save", help="Save distance histogram", action="store_true"
    )
