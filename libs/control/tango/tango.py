...
from libs.control.tango.path_finding.graphs import CostMapGraphNX
...
class TangoControl:
    ...
    def __init__(
        ...
        self.traversable_classes = (
            torch.from_numpy(traversable_classes).to(int).to(self.device)
        )  # [25, 30, 37]
        ...
        self.precomputed_unprojected_no_scale = (
            torch.matmul(self.intrinsics_inv, homogeneous_pts)
        ).T
        ...
        self.grid_shift = torch.tensor(
            [self.h_bev // 2, -3], dtype=torch.long, device=self.device
        )  # todo: make the z roll be auto calculated
    ...
    def compute_goal_point(
        self, depth: torch.Tensor, goal_mask: torch.Tensor
    ) -> torch.Tensor:
        max_depth_indices = torch.where(
            depth == depth[goal_mask == goal_mask.min()].max()
        )
        ...
        point_goal_bev = (unprojected_point / self.grid_size).long()[
            0::2
        ] + self.grid_shift
    ...
    def compute_relative_bev(
        self, traversable: torch.Tensor, depth: torch.Tensor
    ) -> torch.Tensor:
        ...
        xy_t_ij = (
            torch.floor(unprojected_points / self.grid_size).to(torch.long)[:, 0::2]
            + self.grid_shift
        )
        self.occupied_bev.zero_()
        self.free_bev.zero_()
        self.occupied_bev = (
            self.occupied_bev.to(torch.long).index_put_(
                (xy_t_ij[:, 1], xy_t_ij[:, 0]),
        ...
        self.free_bev = (
            self.free_bev.to(torch.long).index_put_(
                (xy_t_ij[:, 1], xy_t_ij[:, 0]), traversable.long(), accumulate=True
            )
            > 0
        ).int()
    ...
    def compute_point_tangents(points: np.ndarray) -> np.ndarray:
        ...
        thetas[-1] = thetas[
            0
        ]  # we dont know which way to face because we have no next point
    ...
    def get_point_poses_numpy(self, path_traversable_bev: np.ndarray) -> np.ndarray:
            ...
            t = (
                np.concatenate(
                    (
                        np.array([0]),
                        np.cumsum(
                            np.diff(traversable_bev_xs, 1) ** 2
                            + np.diff(traversable_bev_zs, 1) ** 2
                        ),
                    )
                )
        ...
        traversable_bev = np.concatenate(
            (traversable_bev_xs[:, None], traversable_bev_zs[:, None]), axis=1
        )
    ...
    def add_safety_margin(self, traversable: torch.Tensor) -> torch.Tensor:
        traversable_with_margin = morph.erosion(
            traversable[None, None, ...], self.kernel_erode
        )
    ...
    def control(
        ...
        if self.check_if_traversable(traversable_relative_bev_safe):
            ...
            cost_map_relative_bev_safe = K.filters.box_blur(
                traversable_relative_bev_safe,
                (5, 5),  # (3, 3)
            ).squeeze(0, 1)  # soften edges to help keep robot from hitting wall

            cost_scaler = 1000
            cost_map_relative_bev_safe = (
                1 - cost_map_relative_bev_safe.squeeze(0, 1)
            ) * cost_scaler + 1

            self.planning_cost_map_relative_bev_safe = (
                cost_map_relative_bev_safe.cpu().numpy()
            )  # for plottin
            ...
            if path_traversable_bev.shape[0] > 1:
                ...
                theta_control = self.pid_steer.control(
                    value_goal=thetai, value_actual=thetaj, time_delta=self.time_delta
                )
