...
class Goal_Gen:
    def __init__(
        ...
        self.cfg = cfg
        self.qIter = -1
        self.goalMask_default = self.max_pl * np.ones(
            (self.H, self.W)
        )  # default value for invalid goal segments
        self.G = G
        ...
        self.nodeID_to_imgRegionIdx = np.array(
            [self.G.nodes[node]["map"] for node in self.G.nodes()]
        )
        ...
        if self.goalNodeIdx == -1:
            ...
            self.goalNodeIdx = utils_data.get_goalNodeIdx(
                self.map_path, self.G, self.nodeID_to_imgRegionIdx, task_type
            )
            if self.goalNodeIdx is None:
                raise ValueError
        self.goalImgIdx = self.nodeID_to_imgRegionIdx[self.goalNodeIdx][
            0
        ]  # TODO: find max if list of goal nodes
        self.cfg.update({"goalImgIdx": self.goalImgIdx})

        self.localizer = loc_topo.LocalizeTopological(
            f"{self.map_path}/images",
            self.G,
            self.W,
            self.H,
            mapImgPositions=self.positions,
            cfg=self.cfg,
        )
        ...
        if self.do_track:
            self.tracker = track_topo.TrackTopological(
                self.W, self.H, self.max_pl, self.cfg
            )
    ...
    def get_goal_mask(
        ...
        if printTime:
            print(f"Segmentation time: {time.time() - t1:.2f}s")
        ...
        if self.qryNodes is None:
            ...
            if printTime:
                print(f"Seg Proc time: {time.time() - t1:.2f}s")

            # LOCALIZE
            self.matchPairs = self.localizer.localize(
                qryImg, self.qryNodes, qryPosition, refImgInds=refImgInds
            )
            if printTime:
                print(f"Localization time: {time.time() - t1:.2f}s")

            # TRACK
            if self.do_track:
                curr_to_matched_pls_history = self.tracker.track(qryImg, self.qryNodes)
            if printTime:
                print(f"Tracking time: {time.time() - t1:.2f}s")
        ...
        # PLAN
        self.pls, nodesClose2Goal = self.planner_g.get_pathLengths_matchedNodes(
            self.matchPairs[:, 1]
        )
        ...
        matchesMask = (
            self.matchPairs[:, 0][:, None] == qryIdx[None, :]
        )  # N_qry_repeat x N_qry_original
        pls_min_inds = (matchesMask * (self.pls.max() - self.pls[:, None])).argmax(
            0
        )  # N_qry_original
        ...
        if printTime:
            print(f"Planning time: {time.time() - t1:.2f}s")

        # TRACK UPDATES PLAN
        if self.do_track:
            self.pls_median, self.local_pls_median = self.tracker.update(
                self.pls_min, qryImg, self.qryNodes, self.localizer.qry_features
            )
        else:
        ...
        if printTime:
            print(f"Tracked Plan & Goal mask time: {time.time() - t1:.2f}s")
    ...
    def visualize_goal_mask(self, qryImg, display: bool = False):
        ...
        vizImg = utils_viz.drawMasksWithColors(
            qryImg, self.qryMasks[self.matchPairs[:, 0]], colors
        )
        if display:
            plt.imshow(vizImg)
            plt.colorbar()
...
if __name__ == "__main__":
    ...
    if segmentor_type == "sam":
        ...
        segmentor = sam2_seg.Seg_SAM2(
            model_checkpoint=modelPath, resize_w=W, resize_h=H
        )
    else:
    ...
    if not os.path.exists(graphPath):
        ...
        logger.warning(
            f"Graph for segmentor {segmentor_type} and matcher {matcher_name} not found. Creating one now!"
        )
        ...
        mapper = map_topo.MapTopological(
            f"{mapPath}/images/", outDir=mapPath, cfg=cfg_map, segmentor=segmentor_type
        )
