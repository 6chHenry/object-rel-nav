...
class LocalizeTopological:
    def __init__(self, imgDir, mapGraph, W, H, mapImgPositions=None, cfg={}):
        ...
        logger.info(f"{len(self.imgNames)} images in the map directory {imgDir}")
        ...
        if self.matcher_name == "lightglue":
            ...
            self.precomputed_map_features_path = (
                Path(imgDir).parent.absolute() / f"map_features_{self.matcher_name}.npy"
            )
            if os.path.exists(self.precomputed_map_features_path):
                logger.info(
                    f"Loading precomputed map features from {self.precomputed_map_features_path}"
                )
        ...
        self.localizer_iter_ub = (
            len(self.imgNames)
            if cfg.get("goalImgIdx", None) is None
            else cfg["goalImgIdx"] + 1
        )
        logger.info(
    ...
    def update_localizer_iter_lb(self):
        if self.greedy_propeller:
            ...
            self.localizer_iter_lb = max(0, self.localizedImgIdx - self.reloc_dia // 2)
    ...
    def localize(self, qryImg, qryNodes, qryPosition=None, refImgInds=None):
        ...
        for refInd in refImgInds:
            ...
            refNodesInds = np.argwhere(
                self.nodeID_to_imgRegionIdx[:, 0] == refInd
            ).flatten()
            ...
            if self.matcher_name == "sam2":
                refNodesList.append(
                    [
                        {
                            "segmentation": rle_to_mask(
                                self.mapGraph.nodes[n]["segmentation"]
                            ),
                            "bbox": self.mapGraph.nodes[n]["bbox"],
                        }
                        for n in refNodesInds
                    ]
                )
            else:
                refNodesList.append(
                    [self.mapGraph.nodes(data=True)[n] for n in refNodesInds]
                )
        ...
        if self.matcher_name == "sam2":
            refImgList_np = [
                cv2.resize(
                    cv2.imread(path)[:, :, ::-1],
                    (self.matcher.resize_w, self.matcher.resize_h),
                )
                for path in refImgList
            ]
            ...
            for i, refIndsLocal in enumerate(matchPairsList.T):
                ...
                matchPairs.append(
                    np.column_stack([qryNodeInds_i, matchedRefNodeInds_i])
                )
            if len(matchPairs) > 0:
            ...
            matchPairsList, _, self.qry_features = (
                self.matcher.matchPair_imgWithMask_multi(
            ...
            if len(matchPairsList) > 0:
                matchedRefNodeInds = np.concatenate(
                    [
                        refNodesIndsList[i][matchPairs[:, 1]]
        ...
        if len(matchPairs) <= self.min_num_matches:
            logger.warning(
                f"Lost! Num matches {len(matchPairs)} < {self.min_num_matches=}."
            )
            self.lost = True
        else:
            self.lost = False
        ...
        if self.try_relocalize and self.lost:
            ...
            if len(refImgInds) == 0 or self.reloc_dia > self.reloc_dia_max:
                logger.info(
                    f"Relocalized exhusted: {len(refImgInds)=}, {self.reloc_dia=}"
                )
                self.reloc_exhausted = True
            else:
                matchPairs = self.relocalize(qryImg, qryNodes, refImgInds)
                matchedRefNodeInds = matchPairs[:, 1]
            self.refImgIndsHistory = np.concatenate(
                [self.refImgIndsHistory, refImgInds]
            )
        ...
        if not self.lost:
            ...
            if len(self.matchedRefNodeIndsHistory) > self.historySize:
                self.matchedRefNodeIndsHistory = self.matchedRefNodeIndsHistory[
                    -self.historySize :
                ]

            matchedRefImgInds = self.nodeID_to_imgRegionIdx[
                np.concatenate(self.matchedRefNodeIndsHistory)
            ][:, 0]
    ...
    def evaluate(
        self, matchPairs, qryNodes, qry_instance_ids, visualize=False, qryImg=None
    ):
        qryMasks = nodes2key([qryNodes[n] for n in matchPairs[:, 0]], "segmentation")
        refNodes = [self.mapGraph.nodes(data=True)[n] for n in matchPairs[:, 1]]
        ...
        for refImgInd in refImgInds:
            instance_ids_ref = np.load(
                f"{self.imgNames[refImgInd].replace('images', 'images_sem').replace('.png', '.npy')}",
                allow_pickle=True,
            )
        ...
        distances = evaluate_segment_association(
            qryMasks,
            refMasks,
            qry_instance_ids[None, ...].repeat(len(qryMasks), axis=0),
            instances2,
        )

        if visualize:
            assert qryImg is not None, (
                "need query image (as rgb array) for visualization"
            )
            idx = distances.argmin()
            visualize_matched_mask_pair(
                qryImg,
                qryMasks[idx],
                cv2.imread(self.imgNames[refImgInds[idx]])[:, :, ::-1],
                refMasks[idx],
            )
...
def evaluate_segment_association(masks1, masks2, instances1, instances2, max_id=1000):
    ...
    """
    assert instances1.max() < max_id and instances2.max() < max_id, (
        f"set {max_id=} to a larger value: {instances1.max()=} {instances2.max()=}"
    )
    ...
    for i in range(masks1.shape[0]):
        ids_dist1 = np.bincount(instance_ids_masks1[i].flatten(), minlength=max_id)
        ids_dist2 = np.bincount(instance_ids_masks2[i].flatten(), minlength=max_id)
        distance = np.linalg.norm(ids_dist1 - ids_dist2)
        pixel_counts = np.maximum(1e-6, np.maximum(ids_dist1.sum(), ids_dist2.sum()))
...
def visualize_matched_mask_pair(
    rgb1, mask1, rgb2, mask2, mask_alpha=0.5, display=False, mask_color=[0, 200, 0]
):
