...
def get_depth_model():
    ...
    path_zoe_depth = (
        Path.cwd() / "model_weights" / "depth_anything_metric_depth_indoor.pt"
    )
...
def get_segmentor(
    ...
    if segmentor_name == "sam":
        ...
        segmentor = sam.Seg_SAM(
            path_models, device, resize_w=image_width, resize_h=image_height
        )
        ...
        segmentor = fast_sam_module.FastSamClass(
            ...
            device=device,
            traversable_categories=traversable_class_names,
        )  # imgsz < 480 gives poorer results

    elif segmentor_name == "sam2":
        from libs.segmentor import sam2_seg

        assert path_models is not None, (
            f"{path_models=} must be provided for {segmentor_name=}!"
        )
        ...
        if "pps" in segmentor_name:
            sam_kwargs = {"points_per_side": int(segmentor_name.split("_")[-1][3:])}
