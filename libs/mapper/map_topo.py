...
logger = logging.getLogger(
    "[Mapper]"
)  # logger level is explicitly set below by LOG_LEVEL (TODO: better option?)
...
class MapTopological:
    def __init__(self, imgDir, outDir, cfg={}, segmentor=None):
        ...
        si, se, ss = (
            self.cfg["subsample_si"],
            self.cfg["subsample_ei"],
            self.cfg["subsample_step"],
        )
        ...
        if len(self.textLabels) > 0:
            assert segmentor_name == "fast_sam", (
                "Text labels only supported for fast_sam"
            )
        ...
        if segmentor is None:
            self.segmentor = model_loader.get_segmentor(
                ...
                self.H,
                device=device,
                path_models=self.cfg.get("modelPath", None),
            )
        else:
        ...
        if self.matcher_name == "lightglue":
            self.matcher = matcher_lg.MatchLightGlue(self.W, self.H, cfg=self.cfg)
        elif self.matcher_name == "sam2":
            self.matcher = self.segmentor

        self.graphPath = (
            f"{self.h5FullPath[:-3]}_graphObject_4_{self.matcher_name}.pickle"
        )
    ...
    def create_map_topo(self):
        if (
            not os.path.exists(self.h5FullPath) and not os.path.exists(self.graphPath)
        ) or self.cfg["force_recompute_masks"]:
        ...
        if (
            not os.path.exists(self.graphPath)
            or self.cfg["force_recompute_graph"]
            or self.cfg["force_recompute_masks"]
        ):
            self.G = self.create_graph_intra_img()
            self.nodeID_to_imgRegionIdx = self.get_nodeID_to_imgRegionIdx(self.G)
        ...
        if self.cfg["precompute_path_lengths"]:
            _ = self.get_precomputed_path_lengths(weight=self.cfg["edge_weight_str"])
    ...
    def get_precomputed_path_lengths(self, weight):
        ...
        if weight not in allPathLengths:
            ...
            allPathLengths.update({weight: self.get_all_paths(self.G4, weight=weight)})
        ...
        with h5py.File(self.h5FullPath, "w") as f:
            ...
            for i, _ in enumerate(tqdm(self.imgNames)):
                ...
                if masks is None:
                    ...
                    masks = [
                        {
                            "segmentation": np.ones((self.H, self.W), dtype=bool),
                            "area": self.H * self.W,
                        }
                    ]
    ...
    def create_graph_intra_img(self, intraNbrsAll=False):
        ...
        with h5py.File(self.h5FullPath, "r") as f:
            for i, _ in enumerate(tqdm(self.imgNames)):
                ...
                nodes = [
                    (
                        numNodesCurr + j,
                        {
                            ...
                            **{
                                k: v[()]
                                for k, v in masks[j].items()
                                if k != "segmentation"
                            },
                            **{
                                "segmentation": utils.mask_to_rle_numpy(
                                    masks_seg[j][None, ...]
                                )[0]
                            },
                        },
                ...
                if intraNbrsAll:
                    edges = [
                        (numNodesCurr + j, numNodesCurr + k)
                        for j in range(numSegs)
                        for k in range(j + 1, numSegs)
                    ]
                else:
                ...
                if i != 0:
                    temporalEdges.append((numNodesCurr - 1, numNodesCurr, {"sim": 0}))
    ...
    def create_graph_inter_img(self):
        ...
        G4 = utils.modify_graph(
            self.G, self.G.nodes(data=True), da_edges_rob + intraImage_edges
        )

        print(f"Num Nodes and eges: {G4.number_of_nodes()}, {G4.number_of_edges()}")
        return G4

    def get_robust_DA_edges(self, topK=None, win=0):
        imgIdx_s, imgIdx_e = (
            self.nodeID_to_imgRegionIdx[0, 0],
            self.nodeID_to_imgRegionIdx[-1, 0] + 1,
        )
        da_edges = []
        temporal_edges = []
        for i in tqdm(range(imgIdx_s, imgIdx_e)):
            ...
            if self.matcher_name == "sam2":
                qryImg = cv2.resize(
                    cv2.imread(self.imgNames[i])[:, :, ::-1],
                    (self.matcher.resize_w, self.matcher.resize_h),
                )
                nodes_i = [
                    {
                        "segmentation": utils.rle_to_mask(node["segmentation"]),
                        "bbox": node["bbox"],
                    }
                    for node in nodes_i
                ]
            ...
            for j in range(i + 1, endIdx):
                nodeInds_j = np.argwhere(
                    self.nodeID_to_imgRegionIdx[:, 0] == j
                ).flatten()
                if len(nodeInds_j) == 0:
                    continue
                ...
                da_edges.append(
                    np.array(
                        [
                            nodeInds_i[matchesBool],
                            nodeInds_j[matches_ij][matchesBool],
                            margin_ij[matchesBool],
                        ]
        ...
        da_edges = [
            (int(e[0]), int(e[1]), {"margin": np.exp(-100 * e[2]), "edgeType": "da"})
            for e in da_edges
        ]
    ...
    def get_all_paths(self, G, weight=None, maxVal=1e6):
        ...
        pathLengths = np.array(
            [
                [pathLengths[src].get(tgt, maxVal) for tgt in G.nodes()]
                for src in G.nodes()
            ]
        ).astype(np.float16)
        return pathLengths
    ...
    def visualize_goal_mask(self, imgIdx, display=False, no_pl=False, goalNodeIdx=None):
        ...
        if self.nodeID_to_imgRegionIdx is None:
            self.nodeID_to_imgRegionIdx = self.get_nodeID_to_imgRegionIdx(self.G4)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 4:
        ...
        assert segmentor_name in ["sam", "sam2"], (
            "modelPath is required for sam and sam2 segmentor_names"
        )
    else:
        modelPath = None
