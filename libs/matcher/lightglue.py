...
class MatchLightGlue:
    def __init__(self, resize_w=320, resize_h=240, device="cuda", cfg={}):
        ...
        self.lexor = (
            SuperPoint(max_num_keypoints=2048, detection_threshold=0.0)
            .eval()
            .to(self.device)
        )
        self.lmatcher = (
            LightGlue(
                features="superpoint", depth_confidence=-1, width_confidence=-1, mp=True
            )
            .eval()
            .to(self.device)
        )
                ...
                with torch.no_grad():
                    ftTgtBatch = {
                        key: torch.from_numpy(
                            np.concatenate(
                                [ftTgtList[r][key] for r in refImgBatch], axis=0
                            )
                        )
                        .to(self.device)
                        .half()
                    ...
                    ftSrcBatch = {
                        key: torch.cat(
                            [ftSrc[key] for _ in range(len(refImgBatch))], dim=0
                        )
                        .to(self.device)
                        .half()
                        for key in ftSrc.keys()
                    }
                    lmatches = self.lmatcher(
                        {"image0": ftSrcBatch, "image1": ftTgtBatch}
                    )
