...
class LightGlue(nn.Module):
    ...
    def _forward(self, data: dict) -> dict:
        ...
        # if nothing left after pruning
        if desc0.shape[-2] == 0 or desc1.shape[-2] == 0:
            m0 = -1 * torch.ones((1, desc0.shape[-2]))
            m1 = -1 * torch.ones((1, desc1.shape[-2]))
            mscores0 = torch.zeros((1, desc0.shape[-2]))
            mscores1 = torch.zeros((1, desc1.shape[-2]))
