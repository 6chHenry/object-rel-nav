...
def showSegFromH5(
    h5filename, ims, imgIdx, regIdx=None, cfg=None, doPlot=True, dataDir="./"
):
    with h5py.File(h5filename, "r") as f:
        ...
        if regIdx is None:
            masks = [
                f[key + f"/masks/{k}/"] for k in natsorted(f[key + "/masks/"].keys())
            ]
        else:
...
def match_all_blobs(image_0, all_masks_0, image_1, all_masks_1, device):
    ...
    matched_kp_numbers = torch.zeros(
        (num_masks_0, num_masks_1), dtype=int, device=device
    )
    matched_kp_scores = torch.zeros(
        (num_masks_0, num_masks_1), dtype=float, device=device
    )

    for i, mask0 in enumerate(all_masks_0):
        mask0_seg = torch.tensor(
            np.array(mask0["segmentation"])
            .reshape(1, im_height, im_width)
            .repeat(3, axis=0),
            device=device,
        )
        ...
        for j, mask1 in enumerate(all_masks_1):
            mask1_seg = torch.tensor(
                np.array(mask1["segmentation"])
                .reshape(1, im_height, im_width)
                .repeat(3, axis=0),
                device=device,
            )
...
def match_query_blob(image_0, query_mask, image_1, all_masks_1, device):
    ...
    query_mask_seg = torch.tensor(
        np.array(query_mask["segmentation"])
        .reshape(1, im_height, im_width)
        .repeat(3, axis=0),
        device=device,
    )
...
if __name__ == "__main__":
    ...
    matched_kp_numbers, matched_kp_scores = match_query_blob(
        image0, masks0[0], image1, masks1, device
    )
    print(
        f"--- Matching a query blob Matrix Estimation: {(time.time() - start_time)} seconds ---"
    )
