...
def local_image_to_data_url(image):
    if isinstance(image, str):
        ...
        base64_encoded_data = base64.b64encode(cv2.imencode(".jpg", image)[1]).decode(
            "utf-8"
        )
    ...
    prompt = [
        {"role": "system", "content": system_prompt},
        {
            ...
            "content": [
                ...
                {
                    "type": "image_url",
                    "image_url": {"url": local_image_to_data_url(image_prompt)},
                },
            ],
        },
    ]
    response = gpt4v_client.chat.completions.create(
        model=deployment_name, messages=prompt, max_tokens=1000
    )
