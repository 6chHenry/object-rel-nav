...
def get_args():
    ...
    parser.add_argument(
        "--difficulty", type=str, choices=["easy", "medium"], default="easy"
    )
...
args = get_args()
if args.prefix == "mp3d":
    habitat_config = mp3d_data_config(
        stage="val",
    ...
    habitat_config = hm3d_data_config(
        stage="val",
        episodes=args.episodes,
        robot_height=args.robot_height,
        robot_radius=args.robot_radius,
...
for i in tqdm(range(args.episodes)):
    os.makedirs(
        "%s_eval_trajectory/evaluate_%s_%d/" % (args.prefix, args.prefix, i),
        exist_ok=True,
    )
    timesteps = 0
    obs = env.reset()
    goal_flag, goal_image, goal_mask, goal_point, goal_dist = random_pixel_goal(
        habitat_config, env, args.difficulty
    )
    ...
    image_writer = imageio.get_writer(
        "%s_eval_trajectory/evaluate_%s_%d/" % (args.prefix, args.prefix, i)
        + "fps.mp4",
        fps=4,
    )
    ...
    while True:
        ...
        move_distance += np.sqrt(
            np.sum(np.square(last_position - env.sim.get_agent_state().position))
        )
        last_position = env.sim.get_agent_state().position
        timesteps += 1
        concat_image = np.concatenate(
            (
                visualize_target(goal_image, goal_mask),
                cv2.cvtColor(pred_mask, cv2.COLOR_BGR2RGB),
            ),
            axis=1,
        )
    ...
    if goal_dist < 3.0:
        sr = (
            np.sqrt(np.sum(np.square(goal_point - env.sim.get_agent_state().position)))
            < 1.0
        )
