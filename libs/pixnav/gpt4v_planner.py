...
class GPT4V_Planner:
    ...
    def concat_panoramic(self, images, angles):
        ...
        background_image = np.zeros(
            (2 * height + 3 * 10, 3 * width + 4 * 10, 3), np.uint8
        )
    ...
    def query_gpt4v(self, pano_images):
        ...
        inference_image = cv2.cvtColor(
            self.concat_panoramic(pano_images, angles), cv2.COLOR_BGR2RGB
        )
        ...
        for i in range(10):
            try:
                ...
                raw_answer = (
                    "{'Reason':'The toilet is visible at Angle %d', 'Angle':%d}"
                    % (angle, angle)
                )
                answer = raw_answer.replace(" ", "")
                answer = answer[answer.index("{") : answer.index("}") + 1]
