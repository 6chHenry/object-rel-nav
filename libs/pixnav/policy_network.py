...
class PixelNav_Policy(nn.Module):
    def __init__(self, max_token_length=64, device="cuda:0"):
        ...
        self.history_backbone = nn.Sequential(
            *(list(imagenet_resnet18().children())[:-1]), nn.Flatten()
        ).to(device)
        # goal encoder to encode both the initial RGB image and the goal mask, 4-channel input
        self.goal_backbone = imagenet_resnet18()
        self.goal_backbone.conv1 = nn.Conv2d(
            4,
            self.goal_backbone.conv1.out_channels,
        ...
        self.goal_backbone = nn.Sequential(
            *(list(self.goal_backbone.children())[:-1]), nn.Flatten()
        ).to(device)
        ...
        self.dt_policy = nn.TransformerDecoder(
            nn.TransformerDecoderLayer(
                768, 4, dropout=0.25, batch_first=True, device=device
            ),
            4,
        )
    ...
    def forward(self, goal_mask, goal_image, episode_image):
        ...
        goal_mask_tensor = (
            torch.as_tensor(goal_mask / 255.0, dtype=torch.float32, device=self.device)
            .permute(0, 3, 1, 2)
            .contiguous()
        )
        goal_image_tensor = (
            torch.as_tensor(goal_image / 255.0, dtype=torch.float32, device=self.device)
            .permute(0, 3, 1, 2)
            .contiguous()
        )
        goal_token = self.goal_backbone(
            torch.concat((goal_image_tensor, goal_mask_tensor), dim=1)
        ).unsqueeze(1)
        ...
        episode_image_tensor = (
            torch.as_tensor(
                episode_image / 255.0, dtype=torch.float32, device=self.device
            )
            .permute(0, 1, 4, 2, 3)
            .contiguous()
        )
        ...
        epc_token = torch.concat(
            (epc_token, goal_concat_token.tile((1, epc_token.shape[1], 1))), dim=-1
        )

        # add the position embedding
        pos_indice = (
            torch.arange(self.max_token_length)
            .expand(epc_token.shape[0], self.max_token_length)
            .to(self.device)
        )
        pos_embed = self.po_embedding(pos_indice)
        epc_token = epc_token + pos_embed
        tgt_mask = generate_square_subsequent_mask(self.max_token_length).to(
            self.device
        )
        out_token = self.dt_policy(
            tgt=epc_token, memory=goal_input_token, tgt_mask=tgt_mask
        )
