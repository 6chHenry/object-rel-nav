...
import quaternion
import time
from libs.pixnav.policy_network import PixelNav_Policy


class Policy_Agent:
    def __init__(
        self,
        model_path="./checkpoints/navigator.ckpt",
        max_token_length=64,
        image_size=224,
        device="cuda:0",
    ):
    ...
    def reset(self, goal_image, goal_mask):
        ...
        self.goal_image = cv2.resize(
            cv2.cvtColor(goal_image, cv2.COLOR_BGR2RGB),
            (self.image_size, self.image_size),
        )
        self.goal_image = self.goal_image[np.newaxis, :, :, :]
        self.goal_mask = cv2.resize(
            goal_mask, (self.image_size, self.image_size), cv2.INTER_NEAREST
        )
    ...
    def step(self, obs_image, collide=False, early_stop=True):
        self.current_obs = cv2.resize(
            cv2.cvtColor(obs_image, cv2.COLOR_BGR2RGB),
            (self.image_size, self.image_size),
        )
        self.history_image[self.predict_length] = (
            self.current_obs.copy()
        )  # append(self.current_obs)
        self.input_image = self.history_image[np.newaxis, :, :, :, :]
        action_pred, distance_pred, goal_pred = self.network(
            self.goal_mask, self.goal_image, self.input_image
        )
        if collide == False:
            return_action = (
                action_pred[0][self.predict_length].detach().cpu().numpy().argmax()
            )
        else:
        ...
        return_goal = cv2.rectangle(
            return_goal,
            (int(goal_x) - 5, int(goal_y) - 5),
            (int(goal_x) + 5, int(goal_y) + 5),
            (255, 0, 0),
            -1,
        )
        return_goal = cv2.putText(
            return_goal,
            "%d" % int(return_distance[0] * 10),
            (310, 50),
