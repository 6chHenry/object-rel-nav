...
def hm3d_config(path: str = HM3D_CONFIG_PATH, stage: str = "val", episodes=200):
    ...
    with read_write(habitat_config):
        ...
        habitat_config.habitat.dataset.data_path = (
            EPISODE_PREFIX + "objectnav/hm3d/v2/{split}/{split}.json.gz"
        )
...
def mp3d_config(path: str = MP3D_CONFIG_PATH, stage: str = "val", episodes=200):
    ...
    with read_write(habitat_config):
        ...
        habitat_config.habitat.dataset.scenes_dir = SCENE_PREFIX
        habitat_config.habitat.dataset.data_path = (
            EPISODE_PREFIX + "objectnav/mp3d/v1/{split}/{split}.json.gz"
        )
        habitat_config.habitat.simulator.scene_dataset = (
            SCENE_PREFIX + "mp3d/mp3d.scene_dataset_config.json"
        )
        habitat_config.habitat.environment.iterator_options.num_episode_sample = (
            episodes
        )
...
def hm3d_data_config(
    ...
    with read_write(habitat_config):
        ...
        habitat_config.habitat.task.measurements = {
            "num_steps": NumStepsMeasurementConfig
        }
...
def mp3d_data_config(
    path: str = MP3D_CONFIG_PATH,
    stage: str = "val",
    episodes=200,
    robot_height=0.88,
    robot_radius=0.25,
    ...
    with read_write(habitat_config):
        ...
        habitat_config.habitat.simulator.scene_dataset = (
            SCENE_PREFIX + "mp3d/mp3d_annotated_basis.scene_dataset_config.json"
        )
        habitat_config.habitat.environment.iterator_options.num_episode_sample = (
            episodes
        )
