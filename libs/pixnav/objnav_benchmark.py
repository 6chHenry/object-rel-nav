...
def adjust_topdown(metrics):
    return cv2.cvtColor(
        colorize_draw_agent_and_fit_to_height(metrics["top_down_map"], 1024),
        cv2.COLOR_BGR2RGB,
    )
...
for i in tqdm(range(args.eval_episodes)):
    ...
    goal_image, goal_mask, debug_image, goal_rotate, goal_flag = nav_planner.make_plan(
        episode_images[-12:]
    )
    ...
    while not habitat_env.episode_over:
        action, skill_image = nav_executor.step(
            obs["rgb"], habitat_env.sim.previous_step_collided
        )
        if action != 0 or goal_flag:
            ...
            for _ in range(11):
                ...
                episode_images.append(obs["rgb"])
                episode_topdowns.append(adjust_topdown(habitat_env.get_metrics()))
            goal_image, goal_mask, debug_image, goal_rotate, goal_flag = (
