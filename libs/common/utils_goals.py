...
warnings.filterwarnings(
    "ignore", category=UserWarning, module="torch.masked.maskedtensor.creation"
)
# python 3.9
warnings.filterwarnings(
    "ignore", category=UserWarning, module="torch.masked.maskedtensor.core"
)

from libs.common import utils


def find_graph_instance_ids_and_path_lengths(
    G4, goal_object_id, device: str = "cpu", weight: str = "margin"
    ...
    if utils.count_edges_with_given_weight(G4, weight) == 0:
        raise ValueError(
            f"No edges found for given {weight=}, found {utils.get_edge_weight_types(G4)=}"
        )
...
def get_goal_mask_GT(
    graph_instance_ids: torch.Tensor,
    pls: torch.Tensor,
    sem: np.ndarray,
    device: str = "cpu",
) -> np.ndarray:
    ...
    method = "new"
    matchedNodeInds_pls = None
    if method == "old":
        matches = sem[:, :, None] == graph_instance_ids[None, None, :]
        ...
        matches = curr_instance_ids[:, None] == graph_instance_ids[None, :]  # N_c x N_g

        # find the minimum path length for each instance
        matches_pls = torch.masked.masked_tensor(
            pls.repeat(matches.shape[0], 1), matches
        )  # N_c x N_g
        matches_pls_min = torch.masked.amin(matches_pls, 1)  # N_c
        matches_pls_min = torch.nan_to_num(
            matches_pls_min, nan=max_val, posinf=max_val, neginf=max_val
        )
