...
import habitat_sim
from libs.common.utils_sim_traj import (
    find_shortest_path,
    get_agent_rotation_from_two_positions,
)
...
def get_final_goal_state_reverse(sim, agent_states):
    ...
    assert (
        np.linalg.norm(
            final_goal_position - agent_states[final_goal_state_idx - 1].position
        )
        >= 0.1
    )
    final_goal_state = habitat_sim.AgentState(
        ...
        rotation=get_agent_rotation_from_two_positions(
            final_goal_position, agent_states[final_goal_state_idx - 1].position
        ),
    )
...
def find_reverse_traverse_goal(
    agent, sim, final_goal_state, map_graph, instance_index_to_name_map=None
):
    ...
    map_graph_insta_ids = np.array(
        [map_graph.nodes[n]["instance_id"] for n in map_graph.nodes]
    )

    goal_object_cat_names_usual = [
        "bed",
        "chair",
        "monitor",
        "plant",
        "sofa",
        "toilet",
    ]  # np.unique([str(ep).split('_')[-3] for ep in episodes])
    filter_objects = False

    goal_object_id = None
    for i, insta_id in enumerate(instance_ids_curr):
        if instance_index_to_name_map[insta_id][1] in [
            "Unknown",
            "floor",
            "ceiling",
            "ceiling lower",
        ...
        if insta_id in map_graph_insta_ids:
            ...
            print(
                f"Selected goal object for reverse traverse: {cat_names_curr[i]} with id: {goal_object_id}"
            )
            break
    if goal_object_id is None:
        raise ValueError(
            "No goal object (common with map_graph) found in the reverse goal mask..."
        )
...
def get_goal_info_alt_goal(mapPath, task_type):
    ...
    seen_but_unvisited_object = np.load(
        f"{mapPath}/{goal_info_filename}", allow_pickle=True
    )[()]
    goal_instance_id = seen_but_unvisited_object["instance_id"]

    goalImgIdx = seen_but_unvisited_object["image_id"]
    instance_mask = np.load(
        f"{mapPath}/images_sem/{goalImgIdx:05d}.npy", allow_pickle=True
    )
...
def get_goal_info(mapPath):
    ...
    goal_filepath = f"{mapPath}/obs_g.npy"
    if os.path.exists(goal_filepath):
        instance_mask = np.load(goal_filepath, allow_pickle=True)[()]["semantic_sensor"]
    else:
        semantic_filepath = f"{mapPath}/images_sem/{goalImgIdx:05d}.npy"
...
def get_goalMaskBinary(mapPath, task_type):
    if task_type in ["original", "via_alt_goal"]:
        ...
        goalImgIdx, instance_mask, goal_instance_id = get_goal_info_alt_goal(
            mapPath, task_type
        )
...
def match_mapMasks_with_goalMask(mapMasks, goalMaskBinary):
    ...
    goalMaskBinary = cv2.resize(
        goalMaskBinary.astype(float), (img_w, img_h), interpolation=cv2.INTER_NEAREST
    ).astype(bool)
...
def get_goalNodeIdx(
    mapPath, G, nodeID_to_imgRegionIdx, task_type, ret_final_goalMask_vis=False
):
    """
    ...
    """
    goalImgIdx, goalMaskBinary, goal_instance_id = get_goalMaskBinary(
        mapPath, task_type
    )

    mapNodeInds_in_goalImg = np.argwhere(
        nodeID_to_imgRegionIdx[:, 0] == goalImgIdx
    ).flatten()
    mapMasks = utils.nodes2key(mapNodeInds_in_goalImg, "segmentation", G).transpose(
        1, 2, 0
    )

    iou_argmax, iou_max = match_mapMasks_with_goalMask(mapMasks, goalMaskBinary)
    if iou_max > 0:
        goalNodeIdx = mapNodeInds_in_goalImg[iou_argmax]
    else:
        goalNodeIdx = None

    if ret_final_goalMask_vis:
        final_goal_mask_vis = cv2.imread(f"{mapPath}/images/{goalImgIdx:05d}.png")
        final_goal_mask_vis[goalMaskBinary] = [255, 0, 0]
        return goalNodeIdx, final_goal_mask_vis
    else:
        return goalNodeIdx
...
def get_goalNodeIdx_reverse(mapPath, G, goal_instance_id):
    ...
    for idx, goalImgIdx in enumerate(goalImgIdxs):
        ...
        iou_argmax, iou_max = match_mapMasks_with_goalMask(
            mapMasks, instance_masks[idx]
        )
        ious.append(iou_max)
    ...
    if np.max(ious) != 0:
        ...
        print(f"{goal_instance_id=} not found")
    return goalNodeIdx
