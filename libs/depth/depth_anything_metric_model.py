...
class DepthAnythingMetricModel(nn.Module):
    def __init__(self, model_name, pretrained_resource):
        ...
        config.pretrained_resource = "local::" + str(
            Path(__file__).parents[0] / pretrained_resource
        )
    ...
    def infer(self, image: np.array):
        ...
        depth = self.depth_anything_model(
            image_tensor / 255.0
        )  # added float right now to get it going

        if isinstance(depth, dict):
            depth = depth.get("metric_depth", depth.get("out"))
        elif isinstance(depth, (list, tuple)):
            depth = depth[-1]
        depth = K.geometry.transform.resize(
            depth,
            (h, w),
