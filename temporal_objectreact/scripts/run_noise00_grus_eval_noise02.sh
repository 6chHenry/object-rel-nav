#!/usr/bin/env bash
# Evaluate the three GRU variants trained with noise_p=0.0 under inference-time
# costmap noise. This reuses existing checkpoints and does not train.
#
# Usage:
#   tmux new -s gru_noise00_eval_noise02
#   cd /path/to/object-rel-nav
#   CUDA_VISIBLE_DEVICES=0 bash temporal_objectreact/scripts/run_noise00_grus_eval_noise02.sh
#
# Optional env:
#   NOISE_PROB=0.2
#   NOISE_SEED=42
...
checkpoint_for() {
  case "$1" in
    gru) echo "logs/temporal_gru/latest.pth" ;;
    gated_gru) echo "logs/temporal_gated_gru_noise00/latest.pth" ;;
    reliability_gated_gru) echo "logs/temporal_reliability_gated_gru_noise00/latest.pth" ;;
...
exp_suffix_for() {
  case "$1" in
    gru) echo "gru_train_noise00_eval_noise02" ;;
    gated_gru) echo "gated_gru_train_noise00_eval_noise02" ;;
    reliability_gated_gru) echo "reliability_gated_gru_train_noise00_eval_noise02" ;;
    *) echo "unknown method: $1" >&2; exit 1 ;;
  esac
}
...
for method in "${methods[@]}"; do
  ...
  for task in "${tasks[@]}"; do
    ...
    echo "[eval-noise00-to-02] checkpoint=$ckpt"
    echo "[eval-noise00-to-02] overrides=${overrides[*]}"

    cmd=("$PY" -m temporal_objectreact.eval_runner -c "$CFG" --set "${overrides[@]}")
    if [[ "${DRY_RUN:-0}" == "1" ]]; then
      printf '[eval-noise00-to-02] DRY_RUN:'
      printf ' %q' "${cmd[@]}"
      printf '\n'
    else
...
echo
if [[ "${DRY_RUN:-0}" == "1" ]]; then
  echo "[eval-noise00-to-02] dry run finished; no evaluations were launched"
else
  echo "[eval-noise00-to-02] aggregating numbers via evaluate_objecreact.py"
